...
Enable visual vertical indicator to avoid lengthy lines (default src_emacs-lisp[:exports results]{(print fill-column)} {{{results(=80=)}}} chars from start)
#+begin_src emacs-lisp
(global-display-fill-column-indicator-mode t)
#+end_src
And set the column to 50 when in Magit's commit message buffer:
#+begin_src emacs-lisp
...
(setq
 ...
 ;; Write the notes in descending order (default)
...
** Windows
#+begin_src emacs-lisp
(general-evil-define-key 'normal 'global :prefix "SPC" "W" 'ace-window)
#+end_src
...
(after! dape
  ...
  (defun dape-vitest/find-file-buffer-default ()
    "Read filename at project root, defaulting to current buffer. Return vector of vitest args to run said file"
    (let ((file (dape-buffer-default)))
      (if file
          `["--no-coverage", file]
        (user-error "No file found"))))

  (defun dape-vitest/ensure (config)
    "Ensure node is available, vitest is installed, that the dapDebugServer is installed"
    ...
    (let ((cwd (dape-cwd))
          ...
          (node-vitest-file (expand-file-name
      ...
      (unless (file-exists-p node-vitest-file)
        (user-error "Vitest executable not found at %S" node-vitest-file))))


  (add-to-list 'dape-configs
               `(vitest
                 modes (js-mode js-ts-mode typescript-mode)
                 ensure dape-vitest/ensure
                 command "node"
                 command-cwd dape-command-cwd
                 command-args (,(expand-file-name
                                 (file-name-concat "/home/user/.config/emacs/debug-adapters/"
                                                   "js-debug"
                                                   "src"
                                                   "dapDebugServer.js"))
                               :autoport)
                 ...
                 :args dape-vitest/find-file-buffer-default
...
Hydra keybindings ([[https://github.com/svaante/dape/wiki#ergonomic-keybindings][Home · svaante/dape Wiki · GitHub]]):
#+begin_src emacs-lisp
...
(after! browse-at-remote
  ...
  (add-to-list 'browse-at-remote-remote-type-regexps
               `(:host "^vf$"
                 :type "gitea"
                 :actual-host ,(getenv "EMACS_VF_HOST")))
  (add-to-list 'browse-at-remote-use-http "vf")
  (add-to-list 'browse-at-remote-remote-type-regexps
               `(:host "^cb$"
                 :type "gitea"
                 :actual-host "codeberg.org")))
#+end_src
...
** Slack
#+begin_src emacs-lisp
(when (string= (getenv "EMACS_ENABLE_SLACK") "true")
  (use-package! emacs-slack
    ...
    :config
    ;; TODO: Instead of conditionally enabling Slack, make it loop over all keys on emacs/slack/*/. The absence means not enabled
    (slack-register-team
...
** IRC
#+begin_src emacs-lisp
(when (string= (getenv "EMACS_ENABLE_IRC") "true")
  (after! circe
    (setq circe-network-options
          `((,(concat "Libera Chat - " (+pass-get-user "emacs/irc"))
             :tls t
             :host "irc.libera.chat"
             :port 6697
             :nick ,(+pass-get-user "emacs/irc")
             :sasl-username ,(+pass-get-user "emacs/irc")
             :sasl-password ,(+pass-get-secret "emacs/irc")
             :channels nil)))))
#+end_src
...
* Kubernetes
#+begin_src emacs-lisp
(general-evil-define-key 'normal 'global-map :prefix "SPC" "k k" 'kubernetes-overview)
#+end_src
